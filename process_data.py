...
# 설정
# ═══════════════════════════════════════
BASE_FILE  = "data/base.xlsx"   # 파일 #1 (고정)
DAILY_FILE = "data/daily.xlsx"  # 파일 #2 (일일 갱신)
OUT_FILE   = "data/processed.json"

HEADER_ROW = 3  # 0-indexed → pd.read_excel header=3

# ═══════════════════════════════════════
# 컬럼 인덱스 정의 (0-based, header행 이후)
# ═══════════════════════════════════════
COL_IDX = {
    "충전기ID":    0,   # A
    "충전소명":    1,   # B  ← 실제 파일에 맞게 수정
    "사이트명":    2,   # C
    "설치장소":    3,   # D
    "상세주소":    4,   # E
    "주소1":       7,   # H
    "누적사용량": 10,   # K  ← 실제 파일에 맞게 수정
    "충전기상태": 15,   # P
    "운영계약시작": 20, # U
    "운영계약종료": 21, # V
    "충전기모델ID": 29, # AD
    ...
    "위도":        40,  # AO
    "경도":        41,  # AP
    "모델분류":    52,  # BA  ← process_and_classify_excel이 기록한 열
    "권역":        53,  # BB
}
